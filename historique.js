    ...
    if (controlTypeBtn) {
        ...
        menuItems.forEach(item => {
            item.addEventListener('click', () => {
                ...
                const btnSpan = controlTypeBtn.querySelector('span');
                btnSpan.textContent = selectedText;
                
                // Mettre à jour l'icône de vérification
                menuItems.forEach(mi => {
                    const checkIcon = mi.querySelector('svg');
                    if (checkIcon) {
                        checkIcon.style.display = mi === item ? 'block' : 'none';
                    }
                });
